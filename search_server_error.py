...
def search_server_start_entry(log_file):
    ...
    with open(log_file, 'r') as file:             
        for row in file:
            if  status_machine == "wait for server start" and re.search(r"server started at http://0.0.0.0:1999" , row)  is not None:
                    date_of_entry = re.match(r"\d{4}-\d{2}-\d{2} (\d{2}:\d{2})", row)
                    status_machine = "wait for program number" 
                    programs_counter = 0
                    continue
            if  status_machine == "wait for program number":
                if programs_counter < 4:
                    programs_counter = programs_counter + 1
                    start_date = date_of_entry[0]
                    start_time = date_of_entry[1]
                    ...
                    if  programs_entry is not None: 
                        ...
                        entry = evaluate_server_start_entry(start_date, start_time, number_of_programs)
                        server_start_list.append(entry)
                        status_machine = "wait for server start"
                else:
                    entry = evaluate_server_start_entry(start_date, start_time,  "None")
                    server_start_list.append(entry)
                    status_machine = "wait for server start"
                    
    return server_start_list

def evaluate_month(server_start_list, outfile):      
...
def main():
    ...
    output_list = evaluate_month(server_start_list, outfile)    
